(function() { // protect the lemmings!
	...
	*/
	...
	const newShoppingListItem = (item, price) => {
		if(typeof item === 'string' || typeof price === 'number') {
			...
			throw new Error('not a string or not a number')
		}
	return {
		'item': item,
		'price' : price
	...
	describe('1. newShoppingListItem', () => {
		...
		/*it('should return an error', () => {
			const shoppingListItem = newShoppingListItem(false, arr)
			chai.assert.throws('not a string or not a number');
		}); */
	});
	...
	*/
	...
	const smartRemoveItems = (i, list = []) => {
		if(i < 0) {
			// given value that does not exist so we absolute.
			list.splice(i, Math.abs(i));
			return list
	...
	*/
	...
	const spliceItems = (items, i, list = []) => {
		if(typeof item !== 'string' || typeof price !== 'number') {
			...
			return addToShoppingList(item, list);
		} else if(i < 0) {
			return list.unshift(items);
		} else {
			return list;
		}
	}
	...
	*/
	const computeSumWithTax = (list = [], taxRate = 8.125) => {
		const answer = totalSum(list) * (1 + taxRate/100);
		return answer;
	}
